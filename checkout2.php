...
if(mysql_num_rows($r1)>0){
	while($s=mysql_fetch_array($r1)){
		...
		$sday=$s['sday'];
		$email=$s['email'];
		$r2=mysql_query("select * from hotel where id='$pid'")or die(mysql_error());
	...
	mysql_query("insert into bookhotel values('','$placename','$room','$sday','$price','$email')")or die(mysql_error());
	mysql_query("delete from demo where pid='$pid'")or die(mysql_error());
	}
	}
	$r3=mysql_query("select * from bookhotel where email='$user'")or die(mysql_error());
	...
	echo"<table  align='center'border='2' width='500px' height='500px' cellspacing='10'>
    	...
    	<tr><th><h3>Name :- <?php echo $placename?></h3></th></tr>
